...
/** Native mobile twin-stick surface. Look and FIRE are separate like main/live touch controls. */
public final class Game3DView extends GLSurfaceView {
    ...
    public Game3DView(Context context,NativeProfileStore store){this(context,store,null);}
    public Game3DView(Context context,NativeProfileStore store,NativeMultiplayerManager network){super(context);setEGLContextClientVersion(3);setPreserveEGLContextOnPause(true);renderer=new Native3DRenderer(context,store,network);setRenderer(renderer);setRenderMode(RENDERMODE_CONTINUOUSLY);setFocusable(true);setFocusableInTouchMode(true);handler.post(resultCheck);}
