...
public final class NativeProfileStore {
    ...
    public synchronized Account active(){
        ...
        if(accounts.isEmpty())throw new IllegalStateException("A device account must be created first.");
    ...
    public synchronized Account createAccount(String displayName){return createAccount(displayName,handleFrom(displayName),"");}
    public synchronized Account createAccount(String displayName,String username,String bio){Account a=defaultAccount();a.displayName=cleanName(displayName);a.username=cleanHandle(username);a.bio=bio==null?"":limit(bio.trim(),90);accounts.add(a);activeId=a.id;save();return a;}
    ...
    private static Account defaultAccount(){Account a=new Account();a.id="acct-"+UUID.randomUUID().toString().substring(0,8);a.username="critter";a.displayName="New Critter";a.bio="Ready for the meadow.";a.speciesId="puppy";a.loadoutId="meadow_scout";a.equippedWeaponId="pea_popper";a.equippedArmorId="leaf_vest";return a;}
    ...
    private static String cleanHandle(String value){String s=value==null?"":value.trim().replaceFirst("^@","").replaceAll("[^A-Za-z0-9_.-]","");return s.isEmpty()?"critter":limit(s,20);}
