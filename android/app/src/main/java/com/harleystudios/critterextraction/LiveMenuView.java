...
public final class LiveMenuView extends View {
    ...
    private void drawReference(Canvas c){refreshDynamicAssets();NativeProfileStore.Account a=store.active();NativeGameData.Loadout kit=a.loadout();NativeGameData.Weapon gun=NativeGameData.WEAPONS.get(a.equippedWeaponId);if(gun==null&&kit!=null)gun=NativeGameData.WEAPONS.get(kit.weaponId);
        round(c,new RectF(0,0,DW,118),0xee0e0f1f,0);if(headerIcon!=null)c.drawBitmap(headerIcon,null,new RectF(64,24,132,92),p);text(c,"Critter Extraction",160,52,34,TEXT,Paint.Align.LEFT,true);text(c,versionLabel()+" • Harley’s Studios • Native 3D Extraction Shooter",160,81,17,MUTED,Paint.Align.LEFT,false);
    ...
    private String versionLabel(){String version=BuildConfig.VERSION_NAME;int suffix=version.indexOf('-');if(suffix>0)version=version.substring(0,suffix);return "v"+version;}
}
