...
const catalog = await readFile(path.join(repo,'live/core/rendering/model-library.js'),'utf8');
const models = await readFile(path.join(repo,'live/core/rendering/species-models.js'),'utf8');
...
for (const [name,source] of [['appearance',appearance],['catalog',catalog],['models',models],['runtimePatch',runtimePatch],['rewardExtension',rewardExtension]]) new vm.Script(source,{filename:`${name}.js`});
...
assert.ok(catalog.includes("['otter','Otter','Cuddle Diver'"),'the shared all-39 catalog registers Otter');
assert.ok(models.includes("id === 'otter'"),'the procedural model library contains Otter-specific geometry');
assert.ok(runtimePatch.includes('runtimeSpeciesAppendSource'),'the generated runtime receives catalog species including Otter');
assert.ok(appearance.includes("otter:{ keys:['otter'"),'Appearance preserves the Otter Critter Code lock');
...
console.log('Issue #59 Otter reward and issue #62 all-39 model integration checks passed.');
