/* Harley's Studios — verified generated-runtime integration for 39 complete critter models. */
(() => {
  'use strict';
  if (window.__NEW_CRITTER_RUNTIME_PATCH_V7__) return;
  window.__NEW_CRITTER_RUNTIME_PATCH_V7__ = true;

  const NativeBlob = window.Blob;
  if (typeof NativeBlob !== 'function') return;

  const MARKER = '__ISSUE_62_COMPLETE_39_RUNTIME_V7__';
  ...
  const BASE_MODEL_START = "renderer.draw('sphere',p.x,.045,p.z,1.22,.035,.92,[.08,.12,.16,.30],p.yaw);";
  const BASE_MODEL_START_PATCHED = `${BASE_MODEL_START}const completeSpeciesModel=window.CritterCompleteModels?.drawCompleteThirdPerson?.({renderer,p,ap,baseY,rightX,rightZ,frontX,frontZ,backX,backZ,dark,paw,body,accent,vest,walk,walkOpp})===true;if(!completeSpeciesModel){`;
  ...
  const report = {
    attempted:0, applied:0, source:'catalog', replacements:[], missing:[],
    delayedRuntime:false, directRuntime:false, lastVerification:null, lastError:''
  };
  ...
  function once(source,name,search,replacement) {
    if (source.includes(replacement)) { markApplied(`${name}:already`); return source; }
    if (!source.includes(search)) { markMissing(name); return source; }
    markApplied(name);
  ...
  function injectSpeciesDatabase(source,additional) {
    ...
    if (!additional) { markMissing('39 species database source'); return source; }
    if (source.includes(CORE_ANCHOR)) {
      markApplied('39 species database');
      return source.replace(CORE_ANCHOR,`${CORE_ANCHOR},\n    ${additional}`);
    }
    const start=source.indexOf('const SPECIES={'),end=start>=0?source.indexOf('};',start):-1;
  ...
  function verification(source) {
    ...
    if(!text.includes('CritterCompleteModels?.drawCompleteThirdPerson'))missing.push('complete third-person model hook');
    if(!text.includes('if(!completeSpeciesModel){'))missing.push('shared Puppy body bypass');
  ...
  function assertPatchedSource(source) {
    ...
    if(!result.complete){
      const error=new Error(`Complete-39 runtime patch incomplete: ${result.missing.join(', ')}`);
      error.name='CritterModelPatchError';report.lastError=error.message;throw error;
    }
    return source;
  }

  function patchSource(source) {
    report.attempted+=1;report.replacements=[];report.missing=[];
    let output=String(source||'');
    if(!output)return output;
    if(output.includes(MARKER)){report.lastVerification=verification(output);return output;}
    output=injectSpeciesDatabase(output,appendSource());
    output=once(output,'species proportions',HEAD_PROFILE,HEAD_PROFILE_PATCHED);
    output=once(output,'skip shared Puppy body',BASE_MODEL_START,BASE_MODEL_START_PATCHED);
    output=once(output,'complete third-person models',DRAW_CHAIN,DRAW_CHAIN_PATCHED);
    ...
    report.applied+=1;report.lastVerification=verification(output);return output;
  }
  ...
  function PatchedBlob(parts=[],options={}) {
    ...
    }catch(error){report.lastError=error?.message||String(error);console.error('[Issue #62] Complete model patch failed.',error);}
  ...
  function loadScript(id,path,required=true){
    return new Promise((resolvePromise,rejectPromise)=>{
      ...
      if(existing){existing.addEventListener('load',()=>resolvePromise(existing),{once:true});existing.addEventListener('error',event=>required?rejectPromise(event):resolvePromise(null),{once:true});return;}
      const script=document.createElement('script');script.id=id;script.src=resolve(path);script.async=false;
  ...
  window.__CRITTER_ISSUE_62_READY__=(async()=>{
    try{
      await loadScript('issue-62-species-models-loader','core/rendering/species-models.js?v=1.2.0-complete-39');
      await loadScript('issue-62-complete-models-loader','core/rendering/species-complete-models.js?v=1.0.0-complete-39');
      await loadScript('issue-62-model-runtime-loader','core/rendering/model-runtime.js?v=2.2.0-complete-39');
      await loadScript('issue-62-live-roster-loader','core/ui/issue-62-live-roster.js?v=2.2.0-complete-39');
      await loadScript('issue-62-live-copy-loader','core/ui/issue-62-live-copy.js?v=1.2.0-complete-39');
      await loadScript('critter-codes-otter-loader','core/rewards/critter-codes-otter.js?v=1.0.0',false);
      const validation=window.CritterCompleteModels?.validateModels?.();
      if(!validation?.ok||validation.count!==39||validation.unique!==39)throw new Error('All 39 complete critter silhouettes did not validate.');
      return true;
    }catch(error){report.lastError=error?.message||String(error);console.error('[Issue #62] Required complete-model integration failed.',error);throw error;}
  })();
  ...
  HTMLHeadElement.prototype.appendChild=function issue62Append(node){
    ...
    if(this===document.head&&/\/core\/game\/game-runtime\.js(?:[?#]|$)/.test(src)&&!node.dataset.issue62Ready&&location.protocol!=='file:'){
      const target=this,original=src;node.dataset.issue62Ready='waiting';report.delayedRuntime=true;
      window.__CRITTER_ISSUE_62_READY__.then(async()=>{
        const response=await fetch(original,{cache:'no-store'});if(!response.ok)throw new Error(`Could not load generated game runtime: HTTP ${response.status}`);
        ...
        node.src=blobUrl;node.dataset.issue62Ready='true';node.dataset.critterCodesPatched='true';report.directRuntime=true;
        node.addEventListener('load',()=>URL.revokeObjectURL(blobUrl),{once:true});node.addEventListener('error',()=>URL.revokeObjectURL(blobUrl),{once:true});
        appendBeforeIssue62Delay.call(target,node);
      }).catch(error=>{report.lastError=error?.message||String(error);node.dataset.issue62Ready='failed';console.error('[Issue #62] Direct complete-model runtime build failed; trying compatibility path.',error);node.src=original;appendBeforeIssue62Delay.call(target,node);});
      return node;
    }
    if(this===document.head&&/\/core\/game\/game-runtime\.js(?:[?#]|$)/.test(src)&&!node.dataset.issue62Ready){
      const target=this;node.dataset.issue62Ready='waiting';report.delayedRuntime=true;
      window.__CRITTER_ISSUE_62_READY__.then(()=>{node.dataset.issue62Ready='true';appendBeforeIssue62Delay.call(target,node);}).catch(error=>{report.lastError=error?.message||String(error);node.dataset.issue62Ready='failed';appendBeforeIssue62Delay.call(target,node);});
      return node;
    }
