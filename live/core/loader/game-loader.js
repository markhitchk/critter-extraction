(() => {
  'use strict';
  const VERSION='2026-08-03-profile-security-v6-legacy-export-boot-fix-2';
  const BASE=`./core/loader/game-loader-base.js?v=${VERSION}`;
  const MODULES=['./core/loader/live-arena-patch-1.js','./core/loader/live-arena-patch-2.js','./core/loader/live-arena-patch-3.js','./core/loader/live-multiplayer-ui-patch.js','./core/loader/live-host-disconnect-patch.js','./core/loader/live-webrtc-stability-patch.js','./core/loader/live-arena-respawn-patch.js','./core/loader/live-minimap-revamp-patch.js','./core/loader/live-inventory-grid-patch.js','./core/loader/live-profile-security-patch.js','./core/loader/live-profile-security-cache-patch.js','./core/loader/live-profile-legacy-export-fix-patch.js'].map(url=>`${url}?v=${VERSION}`);
  ...
  (async()=>{try{for(const url of MODULES)await load(url);window.__CRITTER_ARENA_UI__?.();window.fetch=(input,init)=>{const url=typeof input==='string'?input:String(input?.url||''),request=nativeFetch(input,init);if(!/core\/game\/game-core\.js(?:[?#]|$)/.test(url))return request;return request.then(async response=>{if(!response.ok)return response;let source=await response.text();for(const patch of window.__CRITTER_ARENA_PATCHES__)source=patch(source);return new Response(source,{status:response.status,statusText:response.statusText,headers:response.headers});});};await load(BASE);window.fetch=nativeFetch;}catch(error){window.fetch=nativeFetch;console.error('Critter Extraction live profile security, legacy export, and arena hotfix failed',error);window.__critterBootReport?.('failure',error?.message||String(error));}})();
})();
