(() => {
  ...
  const ACORN_URL = './assets/models/weapons/acorn_sprayer/acorn_sprayer_lod2.glb';

  function replaceRequired(source, find, replacement, label) {
    if (!source.includes(find)) {
      console.warn(`High-end Acorn Sprayer patch skipped: ${label}`);
      return source;
    }
  ...
  function patchAcornSource(source) {
    ...
    const preload = `  if(rendererMode==='webgl')window.HarleyHighEndRuntime?.loadAsset('${ACORN_ID}','${ACORN_URL}').then(parts=>{renderer.installAuthoredGroup?.('${ACORN_ID}',parts);document.documentElement.dataset.acornSprayerModel='ready';}).catch(error=>{console.warn('Authored Acorn Sprayer unavailable',error);document.documentElement.dataset.acornSprayerModel='fallback';});\n\n${rendererReady}`;
    source = replaceRequired(source, rendererReady, preload, 'authored Acorn Sprayer preload');
